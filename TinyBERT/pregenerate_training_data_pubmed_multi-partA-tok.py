...
class DocumentDatabase:
    def __init__(self, num_workers, reduce_memory=False):
        if reduce_memory:
    ...
    def add_document(self, document, multithreading=False):
        if not document:
            return
    ...
    def extend_document(self, document, doc_lengths, multithreading=False):
        # document, doc_lengths are a list. Elements of the list are lines
        if not document:
            return
        if self.reduce_memory:
            raise NotImplemented 
...
def tokenWorker(basePATH, filename, args, destPATH="cache"):
    linecnt = 0
    outFile = open(os.path.join("/", destPATH, f"{filename}-intrim.txt"), 'w')
    #print(f"Processing {filename}-intrim.txt")
    tokenizer = BertTokenizer.from_pretrained(args.bert_model, do_lower_case=args.do_lower_case)
    with open(os.path.join(basePATH, filename)) as f:
        for line in tqdm(f, desc="Loading Dataset %s"%filename, unit=" lines"):
            line = line.strip()
...
def main():
    ...
    fileNameList = [ele for ele in os.listdir(basePATH) if ele.endswith('.json')]
    print(f"len fileNameList: ", len(fileNameList))
    """
    # tokenizing files parallelly
    if args.num_workers > 1: # commented out cuz we already done this - WJ 2020 Dec 09
        print(f"Tokenization - multiproc started with {args.num_workers}")
        tok_workers = Pool(min(args.num_workers, len(fileNameList)))
        arguments = [(basePATH, fileName, args, destPATH) for fileName in fileNameList]
        processedLineCnt = tok_workers.starmap(tokenWorker, arguments)
    ...
    if args.num_workers > 1:
        ...
        for thread in threading.enumerate():
            if thread is not mainThread:
                thread.join()
        print('Wating for writing threads to be executed.')
    else:
